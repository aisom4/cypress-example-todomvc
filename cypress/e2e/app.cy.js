...
describe("TodoMVC - React", function () {
  ...
  context("Contrast", () => {
    ...
    it("has good contrast with several todos", () => {
      cy.addAxeCode();
      cy.get(".new-todo").type("learn testing{enter}").type("be cool{enter}");
      //ADDING IN THREE NEW TO DO ITEM TO THE LIST-------------------------------------------------------------------------------------------------------
      cy.get(".new-todo")
      ...
      cy.get(".todo-list li")
        .eq(4) // Get the third item (index starts at 0)
        .find(".toggle") // Find the checkbox element
        .check(); // Mark the checkbox as complete

      //--------------------------------------------------------------------------------------------
      cy.get(".todo-list li").should("have.length", 5); //UPDATED THE LENGTH TO ACCOMMODATE FOR 5 ITEMS TO AVOID ERRORS-------------------------------------
      ...
      //TEST TO SEE IF "LEARN CYPRESS IS COMPLETE ------------------------------------------------------------------------------
      cy.get(".todo-list li")
        .eq(4)
        .contains("Learn Cypress") // Find the todo item containing "Learn Cypress" text
        .parent() // Traverse to the parent <li> element
        .should("have.class", "completed"); // Verify that the todo item has the "completed" class
